...
Table-backed parameter queries compile only when they select plain columns from one public-schema table, with a `WHERE` conjunction of column-equals-binding terms (auth, request, or subscription bindings; multiple terms form a composite key) and optional column `IS [NOT] NULL` or column-equals-string-literal row predicates, and any request-side predicate mirrored by the lifted stream request filter. Bootstrap additionally requires each lookup table to have an `id` column and requires that column and every key, selected, and row-predicate column to be PostgreSQL `text` or `varchar`, because the snapshot and replication paths only agree byte-for-byte on text-family values; violating either boundary fails compilation or bootstrap rather than serving degraded results. Bucket membership resolved through these lookups is still evaluated once per `/sync/stream` request; a change to access rows takes effect on the client's next reconnect, not mid-stream.
