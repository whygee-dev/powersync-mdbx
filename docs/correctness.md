...
Parameter-lookup entries for table-backed queries are written in the same MDBX transaction as the batch's other writes and the persisted LSN watermark, so a lookup update and the corresponding checkpoint position become durable together or not at all. An update or delete that moves a row out of a lookup key's prior value removes the corresponding lookup entry before any new one is written, so the row stops resolving through the old key. Parameter-lookup entries are part of the storage contract like other materialized layouts: adding, removing, or changing a table-backed parameter query changes that contract, so an existing MDBX store fails the layout-version check and is wiped and re-materialized from a fresh snapshot on next boot rather than migrated in place.
